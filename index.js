...
/*
 * ============================================================
 * BRAIN STUDIO — RÉPARATION HISTORIQUE SANS RECALCUL
 * ============================================================
 *
 * Cette réparation ne relance jamais l'analyse du match.
 * Elle utilise uniquement les marchés déjà enregistrés dans
 * studio_snapshot afin d'éviter toute fuite de données après
 * le coup d'envoi.
 */
function getStoredStudioMarkets(snapshot = {}) {
  if (!snapshot || typeof snapshot !== "object") {
    return [];
  }

  return Array.isArray(snapshot.markets)
    ? snapshot.markets.filter(Boolean)
    : [];
}

function selectStoredHighestDecisionScoreMarket(markets = []) {
  return [...markets].sort((a, b) => {
    const scoreDifference =
      studioNumber(b?.decision?.score ?? b?.decisionScore ?? b?.score) -
      studioNumber(a?.decision?.score ?? a?.decisionScore ?? a?.score);
    ...
    return (
      studioNumber(
        b?.fairOdds?.calibratedProbability ?? b?.calibratedProbability ?? b?.probability
      ) -
      studioNumber(
        a?.fairOdds?.calibratedProbability ?? a?.calibratedProbability ?? a?.probability
      )
    );
  })[0] || null;
}

async function repairStoredStudioSnapshot(fixtureId) {
  ...
  if (!prediction) {
    throw new Error("Prédiction Railway introuvable");
  }

  const snapshot =
    prediction.studio_snapshot && typeof prediction.studio_snapshot === "object"
      ? prediction.studio_snapshot
      : {};

  const markets = getStoredStudioMarkets(snapshot);
  ...
  if (markets.length === 0) {
    return {
      ...
      repaired: false,
      skipped: true,
      reason: "STORED_MARKETS_MISSING",
    };
  }

  const primaryMarket =
    selectStoredHighestDecisionScoreMarket(markets);
  ...
  if (!primaryMarket) {
    return {
      ...
      repaired: false,
      skipped: true,
      reason: "PRIMARY_MARKET_MISSING",
    };
  }

  const previousMarketKey =
    prediction.studio_market_key || null;
  const previousDecisionScore =
  ...
  const repairedSnapshot = {
    ...
    repairMode: "stored-markets-only",
  };
  ...
  const saved = await saveStudioSnapshot({
    ...
    probability:
      ...
      primaryMarket?.calibratedProbability ??
      primaryMarket?.probability,
    decisionScore:
      ...
      primaryMarket?.score,
    decisionType:
      primaryMarket?.decision?.type ??
      primaryMarket?.decisionType,
    decisionGrade:
      primaryMarket?.decision?.grade ??
      primaryMarket?.decisionGrade,
    analysisVersion:
      snapshot.version ||
      snapshot.analysisVersion ||
      "brain-studio-repair-v1",
  ...
  return {
    ...
    skipped: false,
    changed:
      previousMarketKey !== saved.studio_market_key ||
      previousDecisionScore !== Number(saved.studio_decision_score),
    previousMarketKey,
    previousDecisionScore,
    primaryMarket,
    prediction: saved,
  };
}
...
app.post(
  "/internal/repair-studio-snapshot/:fixtureId",
  async (req, res) => {
    try {
      const result = await repairStoredStudioSnapshot(
        req.params.fixtureId
      );
      ...
      console.error(
        "ERREUR RÉPARATION SNAPSHOT BRAIN STUDIO :",
        error
      );
...
app.post(
  "/internal/repair-studio-snapshots/today",
  async (req, res) => {
    try {
      const result = await pool.query(`
        SELECT fixture_id
        ...
        WHERE fixture_date IS NOT NULL
          AND DATE(
            fixture_date AT TIME ZONE 'Europe/Paris'
          ) = DATE(
      ...
      for (const row of result.rows) {
        try {
          details.push(
            await repairStoredStudioSnapshot(
              row.fixture_id
            )
          );
        } catch (error) {
      ...
      const repaired = details.filter(
        (item) => item.repaired === true
      ).length;
      ...
      console.error(
        "ERREUR RÉPARATION SNAPSHOTS DU JOUR :",
        error
      );
