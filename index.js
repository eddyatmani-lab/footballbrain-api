...
/*
 * ============================================================
 * FOOTBALLBRAIN CORE V2
 * EVENT ENGINE
 * ============================================================
 */

const FOOTBALLBRAIN_CORE_VERSION =
  "footballbrain-core-v2";

/*
 * Création de la table centrale des événements IA.
 */
async function ensureAIEventTables() {
  await pool.query(`
  ...
  console.log(
    "✅ FOOTBALLBRAIN CORE : table ai_events vérifiée"
  );
}

/*
 * Transforme une valeur JavaScript en JSON
 * compatible avec PostgreSQL.
 */
function normalizeAIEventJSON(value) {
  if (
    value === undefined ||
    value === null
  ) {
 ...
 *
 * Exemple :
 * "studio snapshot saved"
 * devient :
 * "STUDIO_SNAPSHOT_SAVED"
 */
function normalizeAIEventName(
  value,
  fallback = "UNKNOWN"
) {
  const normalized = String(
...
/*
 * Fonction centrale utilisée plus tard par :
 *
 * - Brain Studio ;
 * - Learning Engine ;
 * - Timeline ;
...
async function emitAIEvent({
  ...
  if (!eventType) {
    throw new Error(
      "emitAIEvent : eventType manquant"
    );
  }

  if (!eventCategory) {
    throw new Error(
      "emitAIEvent : eventCategory manquante"
    );
  }
  ...
  if (
    ...
    throw new Error(
      "emitAIEvent : fixtureId invalide"
    );
  }

  const normalizedEventType =
    normalizeAIEventName(eventType);

  const normalizedCategory =
    normalizeAIEventName(
      eventCategory
    );
...
/*
 * Lecture de la Timeline d’un match.
 */
async function getFixtureAIEvents(
  fixtureId,
  limit = 100
) {
  const normalizedFixtureId =
...
/*
 * Route temporaire permettant de tester
 * l’enregistrement d’un événement.
 *
 * Cette route sera supprimée lorsque
 * l’Event Engine sera connecté aux vrais moteurs.
 */
app.get(
  "/internal/test-ai-event/:fixtureId",
  async (req, res) => {
    try {
      await ensureAIEventTables();
      ...
      const event =
        await emitAIEvent({
          ...
          metadata: {
            message:
              "Premier événement FootballBrain Core V2",
      ...
      return res.json({
        ...
        message:
          "Event Engine opérationnel",

        event,
      });
    } catch (error) {
      console.error(
        "ERREUR TEST AI EVENT :",
        error
      );
...
/*
 * Première route publique de Timeline.
 */
app.get(
  "/public/ai-timeline/:fixtureId",
  async (req, res) => {
    try {
      await ensureAIEventTables();
      ...
      return res
        ...
        .json({
          ...
          error:
            error?.message ||
            "Impossible de charger la Timeline IA",
        });
    }
  }
);
